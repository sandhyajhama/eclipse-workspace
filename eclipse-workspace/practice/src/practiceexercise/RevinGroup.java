	...
	static void reverse(int array[], int len, int k)
		{
			for (int i = 0; i < len; i = i+ k)		//here for outer loop we increase i with i+k every time.
			{
				...
				//n is not fully divided by k, than for reversing last group, right will be smaller of 
				
				int right = ((i + k - 1< len - 1)?(i+k-1):len-1) ;	//Lambda function or ternary operator.
